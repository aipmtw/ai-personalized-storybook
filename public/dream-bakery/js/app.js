...
const pages = [
  ...
  },
  {
    ...
    emoji: '\uD83D\uDC67\u2728\uD83C\uDFE0',
    zh: '小麥是一個愛做夢的女孩。她最大的夢想，就是有一天能開一間屬於自己的麵包店，讓整條街都飄著香噴噴的麵包味。',
    en: 'Xiao Mai was a girl who loved to dream. Her biggest dream was to one day open her very own bakery, filling the whole street with the warm smell of fresh bread.'
  },
  {
    type: 'story',
    bg: 'page-bg-3',
    ...
    emoji: '\uD83D\uDCC5\uD83E\uDDD1\u200D\uD83C\uDF73\uD83C\uDF5E',
    zh: '每個週末，小麥都會早早起床，把廚房變成她的小小烘焙工坊。她跟著食譜一步一步學，量麵粉、揉麵糰、等發酵。',
    en: 'Every weekend, Xiao Mai woke up early and turned the kitchen into her little baking workshop. She followed recipes step by step — measuring flour, kneading dough, and waiting for it to rise.'
  },
  {
    type: 'story',
    bg: 'page-bg-4',
    ...
    emoji: '\uD83E\uDED8\uD83D\uDE2D\uD83D\uDCA8',
    zh: '第一次烤出來的麵包，硬得像石頭一樣！小麥咬了一口，牙齒都快斷了。她好失望，但她告訴自己：「沒關係，再試一次就好。」',
    en: 'Her first loaf came out hard as a rock! Xiao Mai took a bite and nearly broke her teeth. She was so disappointed, but she told herself, "It\'s okay, I\'ll just try again."'
  },
  {
    type: 'story',
    bg: 'page-bg-5',
    ...
    emoji: '\uD83D\uDCC8\uD83C\uDF5E\u2728',
    zh: '她不斷地練習，每一次都比上一次進步一點點。麵包從硬邦邦變成鬆軟的，從沒味道變成香甜可口。小麥的手越來越巧了。',
    en: 'She kept practicing, improving a little bit each time. Her bread went from rock-hard to soft and fluffy, from tasteless to sweet and delicious. Xiao Mai\'s hands grew more and more skilled.'
  },
  {
    type: 'story',
    bg: 'page-bg-6',
    ...
    emoji: '\uD83C\uDFE0\uD83E\uDDF4\uD83C\uDF5E',
    zh: '小麥在家門前擺了一張小桌子，鋪上格子布，把剛烤好的麵包整齊排好。她做了一個小小的招牌：「小麥的夢想麵包店」。',
    en: 'Xiao Mai set up a tiny table in front of her house, covered it with a checkered cloth, and neatly arranged her freshly baked bread. She made a little sign: "Xiao Mai\'s Dream Bakery."'
  },
  {
    type: 'story',
    bg: 'page-bg-7',
    ...
    emoji: '\uD83D\uDCEE\uD83D\uDE0A\uD83C\uDF5E',
    zh: '第一位客人是每天來送信的郵差叔叔。他拿起一個麵包，咬了一口，露出大大的笑容：「哇，好好吃！小麥，你真的很厲害耶！」',
    en: 'Her first customer was the mailman who came by every day. He picked up a loaf, took a bite, and broke into a big smile: "Wow, this is delicious! Xiao Mai, you\'re really amazing!"'
  },
  {
    type: 'story',
    bg: 'page-bg-8',
    ...
    emoji: '\uD83C\uDFE1\uD83D\uDC68\u200D\uD83D\uDC69\u200D\uD83D\uDC67\uD83C\uDF5E',
    zh: '消息很快就傳開了。隔壁的阿姨、對面的爺爺、樓上的小朋友，大家都跑來買小麥的麵包。她還發明了一種「夢想麵包」，裡面藏著不同的驚喜餡料！',
    en: 'Word spread quickly. The auntie next door, the grandpa across the street, the kids upstairs — everyone came to buy Xiao Mai\'s bread. She even invented a special "Dream Bread" with surprise fillings inside!'
  },
  {
    type: 'story',
    bg: 'page-bg-9',
    ...
    emoji: '\uD83D\uDCB0\uD83E\uDDC1\u2728',
    zh: '小麥把賺到的錢存起來，終於買了一套真正的烘焙工具——專業的攪拌盆、量杯和擀麵棍。她開心地抱著新工具，覺得夢想又靠近了一步。',
    en: 'Xiao Mai saved up the money she earned and finally bought a set of real baking tools — a professional mixing bowl, measuring cups, and a rolling pin. She hugged her new tools happily, feeling one step closer to her dream.'
  },
  {
    type: 'end',
    bg: 'page-bg-10',
    ...
    emoji: '\uD83C\uDF1F\uD83C\uDF5E\uD83D\uDC96',
    zh: '小麥學到了一件很重要的事：每一個大夢想，都是從一個小小的步驟開始的。只要不放棄，總有一天，夢想會像麵包一樣，慢慢膨脹，變得又大又美好。',
    en: 'Xiao Mai learned something very important: every big dream starts with one small step. As long as you never give up, someday your dream will rise — just like bread — growing bigger and more wonderful.'
  }
];
...
function lineLoginFromGate() {
  ...
  const returnUrl = encodeURIComponent(window.location.origin + '/dream-bakery/book.html');
...
function updateUI() {
  ...
  const colors = ['#8d6e63','#efebe9','#fff3e0','#fbe9e7','#fff8e1','#efebe9','#fff3e0','#fce4ec','#e8f5e9','#fff8e1'];
...
function toggleAudio() {
  ...
  if (langMode === 'both' || langMode === 'zh') {
    files.push(`/dream-bakery/audio/page-${pageNum}-zh.mp3`);
  }
  if (langMode === 'both' || langMode === 'en') {
    files.push(`/dream-bakery/audio/page-${pageNum}-en.mp3`);
  }
  ...
  for (let i = 0; i < totalDurations.length; i++) {
    ...
    if (cumulative + d >= target) {
      ...
      if (i === currentFileIndex && currentAudio) {
        ...
        const pageNum = currentPage + 1;
        const files = [];
        if (langMode === 'both' || langMode === 'zh') files.push(`/dream-bakery/audio/page-${pageNum}-zh.mp3`);
        if (langMode === 'both' || langMode === 'en') files.push(`/dream-bakery/audio/page-${pageNum}-en.mp3`);
...
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.register('/dream-bakery/sw.js', { updateViaCache: 'none' });
}
