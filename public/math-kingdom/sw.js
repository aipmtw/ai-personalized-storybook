...
const AUDIO_URLS = [];
for (let i = 1; i <= 12; i++) {
  AUDIO_URLS.push(`/math-kingdom/audio/page-${i}-zh.mp3`);
  AUDIO_URLS.push(`/math-kingdom/audio/page-${i}-en.mp3`);
}
  ...
  if (event.request.mode === 'navigate' || event.request.url.endsWith('.html')) {
    event.respondWith(
      fetch(event.request).then((response) => {
        ...
        return response;
      }).catch(() => caches.match(event.request) || caches.match('/math-kingdom/book.html'))
    );
    return;
  }
