const CACHE_NAME = 'storybook-star-musician-v4';

// Generate audio file URLs for all 12 pages (cover + 10 story + end)
const AUDIO_URLS = [];
for (let i = 1; i <= 12; i++) {
  AUDIO_URLS.push(`/star-musician/audio/page-${i}-zh.mp3`);
  AUDIO_URLS.push(`/star-musician/audio/page-${i}-en.mp3`);
}
  ...
  if (event.request.mode === 'navigate' || event.request.url.endsWith('.html') || event.request.url.includes('/shared/')) {
    event.respondWith(
      fetch(event.request).then((response) => {
        ...
        return response;
      }).catch(() => caches.match(event.request) || caches.match('/star-musician/book.html'))
    );
    return;
  }
