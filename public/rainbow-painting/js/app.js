...
const pages = [
  ...
  },
  {
    ...
    emoji: '👧🖌️✨',
    zh: '小彩是一個愛畫畫的小女孩。有一天，她發現了一支神奇的畫筆。',
    en: 'Little Xiao Cai is a girl who loves to paint. One day, she found a magical paintbrush.'
  },
  {
    type: 'story',
    bg: 'page-bg-3',
    ...
    emoji: '🐦❤️🌤️',
    zh: '她用紅色畫了一隻小鳥。小鳥動了動翅膀，真的飛走了！',
    en: 'She painted a red bird. The bird flapped its wings and really flew away!'
  },
  {
    type: 'story',
    bg: 'page-bg-4',
    ...
    emoji: '💧🌊🏠',
    zh: '她又畫了一條藍色的小河。河水嘩嘩地流過房間的地板。',
    en: 'She then painted a blue river. The water splashed and flowed across the room.'
  },
  {
    type: 'story',
    bg: 'page-bg-5',
    ...
    emoji: '🌳🌲🍃',
    zh: '小彩拿起綠色，畫了一片大森林。房間裡長出了好多大樹！',
    en: 'Xiao Cai picked up green and painted a big forest. Many tall trees grew inside the room!'
  },
  {
    type: 'story',
    bg: 'page-bg-6',
    ...
    emoji: '☀️🌈😊',
    zh: '她畫了一個黃色的太陽。溫暖的陽光照在每個人身上。',
    en: 'She painted a yellow sun. Warm sunlight shone on everyone.'
  },
  {
    type: 'story',
    bg: 'page-bg-7',
    ...
    emoji: '🎨💦🌈',
    zh: '哎呀！小彩不小心打翻了所有顏色。顏色混在一起，變成了一道彩虹！',
    en: 'Oops! Xiao Cai accidentally spilled all the colors. They mixed together and became a rainbow!'
  },
  {
    type: 'story',
    bg: 'page-bg-8',
    ...
    emoji: '🌈👧☁️',
    zh: '小彩跳上彩虹，飛過天空。她看到了整個小鎮，好美啊！',
    en: 'Xiao Cai jumped on the rainbow and flew across the sky. She saw the whole town — so beautiful!'
  },
  {
    type: 'story',
    bg: 'page-bg-9',
    ...
    emoji: '👨‍👩‍👧💕🏠',
    zh: '小彩回到家，畫了爸爸、媽媽和自己。這是她最喜歡的一幅畫。',
    en: 'Xiao Cai came home and painted her dad, mom, and herself. This was her favorite painting.'
  },
  {
    type: 'end',
    bg: 'page-bg-10',
    ...
    emoji: '❤️🎨🌟',
    zh: '小彩學到了：最美的畫，是用心畫出來的。',
    en: 'Xiao Cai learned that the most beautiful art comes from the heart.'
  }
];
...
function updateUI() {
  ...
  const colors = ['#ff7043','#ef5350','#42a5f5','#66bb6a','#ffca28','#ff7043','#7e57c2','#29b6f6','#ff8a65','#ec407a'];
  document.querySelector('meta[name="theme-color"]').content = colors[currentPage] || '#ff7043';
}
...
function toggleAudio() {
  ...
  if (langMode === 'both' || langMode === 'zh') {
    files.push(`/rainbow-painting/audio/page-${pageNum}-zh.mp3`);
  }
  if (langMode === 'both' || langMode === 'en') {
    files.push(`/rainbow-painting/audio/page-${pageNum}-en.mp3`);
  }
  ...
  for (let i = 0; i < totalDurations.length; i++) {
    ...
    if (cumulative + d >= target) {
      ...
      if (i === currentFileIndex && currentAudio) {
        ...
        const pageNum = currentPage + 1;
        const files = [];
        if (langMode === 'both' || langMode === 'zh') files.push(`/rainbow-painting/audio/page-${pageNum}-zh.mp3`);
        if (langMode === 'both' || langMode === 'en') files.push(`/rainbow-painting/audio/page-${pageNum}-en.mp3`);
...
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.register('/rainbow-painting/sw.js', { updateViaCache: 'none' });
}
