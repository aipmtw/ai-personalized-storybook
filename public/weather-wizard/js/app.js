...
const pages = [
  ...
  },
  {
    ...
    emoji: '\uD83D\uDC67\u2601\uFE0F\uD83D\uDC40',
    zh: '小天是個喜歡抬頭看天空的女孩。每天早上出門前，她都會看看雲的形狀，猜猜今天會是什麼天氣。她總是猜得很準。',
    en: 'Xiao Tian was a girl who loved looking up at the sky. Every morning before leaving home, she would study the shape of the clouds and guess what the weather would be. She was almost always right.'
  },
  {
    type: 'story',
    bg: 'page-bg-3',
    ...
    emoji: '\uD83D\uDE12\uD83D\uDE44\u2753',
    zh: '「你又在編故事了吧？」同學們笑她，「看雲怎麼可能知道天氣？」小天有點難過，但她知道自己看到的是真的。',
    en: '"You\'re making things up again!" her classmates laughed. "How can you know the weather just by looking at clouds?" Xiao Tian felt a little sad, but she knew what she saw was real.'
  },
  {
    type: 'story',
    bg: 'page-bg-4',
    ...
    emoji: '\uD83C\uDF27\uFE0F\u2614\uD83D\uDE04',
    zh: '有一天早上，小天看到天空佈滿厚厚的灰色雲層。「今天一定會下雨！」她帶了好幾把雨傘到學校，分給沒帶傘的同學。果然，下午就下起大雨了。',
    en: 'One morning, Xiao Tian saw thick gray clouds covering the sky. "It\'s definitely going to rain today!" She brought several umbrellas to school and shared them with classmates who didn\'t have one. Sure enough, it poured that afternoon.'
  },
  {
    type: 'story',
    bg: 'page-bg-5',
    ...
    emoji: '\uD83D\uDC69\u200D\uD83C\uDFEB\uD83E\uDD14\u2728',
    zh: '老師很驚訝，問小天：「你怎麼知道今天會下雨？」小天紅著臉說：「因為……我會看雲。不同的雲代表不同的天氣。」',
    en: 'The teacher was amazed and asked, "How did you know it would rain today?" Xiao Tian blushed and said, "Because... I can read the clouds. Different clouds mean different weather."'
  },
  {
    type: 'story',
    bg: 'page-bg-6',
    ...
    emoji: '\u2601\uFE0F\u2600\uFE0F\uD83C\uDF24\uFE0F',
    zh: '小天在黑板上畫了三種雲。「胖胖的積雲像棉花糖，表示好天氣。薄薄的捲雲像絲線，代表天氣要變了。又厚又暗的雲，就是要下雨囉！」',
    en: 'Xiao Tian drew three types of clouds on the blackboard. "Fluffy cumulus clouds look like cotton candy — they mean sunny weather. Thin cirrus clouds like silk threads mean change is coming. Thick, dark clouds mean rain is on the way!"'
  },
  {
    type: 'story',
    bg: 'page-bg-7',
    ...
    emoji: '\uD83C\uDF08\uD83D\uDE32\u2728',
    zh: '有天暴風雨過後，小天看著天空說：「等一下會出現彩虹！」同學們半信半疑。十分鐘後，一道美麗的彩虹真的出現在天空中。',
    en: 'One day after a storm, Xiao Tian looked at the sky and said, "A rainbow is about to appear!" Her classmates were half-skeptical. Ten minutes later, a beautiful rainbow truly appeared across the sky.'
  },
  {
    type: 'story',
    bg: 'page-bg-8',
    ...
    emoji: '\uD83D\uDC4F\uD83C\uDF0D\uD83D\uDCCB',
    zh: '全班都看到了那道彩虹，大家歡呼起來。從那天起，同學們開始叫小天「天氣魔法師」。她邀請大家一起成立了「天氣觀察社」。',
    en: 'The whole class watched the rainbow and cheered. From that day on, everyone called Xiao Tian the "Weather Wizard." She invited everyone to start a "Weather Observation Club" together.'
  },
  {
    type: 'story',
    bg: 'page-bg-9',
    ...
    emoji: '\uD83C\uDF21\uFE0F\uD83D\uDCA8\uD83D\uDCDD',
    zh: '天氣觀察社的同學們一起動手做了一個簡易氣象站——用寶特瓶做雨量計，用風車測風向。每天記錄天氣，比比看誰預測得最準。',
    en: 'The club members built a simple weather station together — a rain gauge from a plastic bottle and a pinwheel to measure wind direction. They recorded the weather every day and competed to see who could predict it best.'
  },
  {
    type: 'end',
    bg: 'page-bg-10',
    ...
    emoji: '\uD83D\uDD2D\uD83C\uDF3F\uD83D\uDE0A',
    zh: '小天對同學們說：「科學其實不難，就是認真觀察大自然。只要你願意看、願意想，每個人都可以成為天氣魔法師！」',
    en: 'Xiao Tian told her classmates, "Science isn\'t really that hard — it\'s just carefully observing nature. As long as you\'re willing to look and think, everyone can become a Weather Wizard!"'
  }
];
...
function lineLoginFromGate() {
  ...
  const returnUrl = encodeURIComponent(window.location.origin + '/weather-wizard/book.html');
...
function updateUI() {
  ...
  const colors = ['#5c6bc0','#e3f2fd','#eceff1','#e8eaf6','#b0bec5','#e1f5fe','#fff8e1','#e8f5e9','#fce4ec','#e0f7fa'];
  document.querySelector('meta[name="theme-color"]').content = colors[currentPage] || '#5c6bc0';
}
...
function toggleAudio() {
  ...
  if (langMode === 'both' || langMode === 'zh') {
    files.push(`/weather-wizard/audio/page-${pageNum}-zh.mp3`);
  }
  if (langMode === 'both' || langMode === 'en') {
    files.push(`/weather-wizard/audio/page-${pageNum}-en.mp3`);
  }
  ...
  for (let i = 0; i < totalDurations.length; i++) {
    ...
    if (cumulative + d >= target) {
      ...
      if (i === currentFileIndex && currentAudio) {
        ...
        const pageNum = currentPage + 1;
        const files = [];
        if (langMode === 'both' || langMode === 'zh') files.push(`/weather-wizard/audio/page-${pageNum}-zh.mp3`);
        if (langMode === 'both' || langMode === 'en') files.push(`/weather-wizard/audio/page-${pageNum}-en.mp3`);
...
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.register('/weather-wizard/sw.js', { updateViaCache: 'none' });
}
