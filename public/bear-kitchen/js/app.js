...
function updateUI() {
  ...
  const colors = ['#f4845f','#ffb347','#f9cb9c','#e57373','#ce93d8','#ffcc80','#a5d6a7','#ff8a65','#ffb74d','#8d6e63'];
  document.querySelector('meta[name="theme-color"]').content = colors[currentPage] || '#f4845f';
}
...
function toggleAudio() {
  ...
  if (langMode === 'both' || langMode === 'zh') {
    files.push(`/bear-kitchen/audio/page-${pageNum}-zh.mp3`);
  }
  if (langMode === 'both' || langMode === 'en') {
    files.push(`/bear-kitchen/audio/page-${pageNum}-en.mp3`);
  }
  ...
  for (let i = 0; i < totalDurations.length; i++) {
    ...
    if (cumulative + d >= target) {
      ...
      if (i === currentFileIndex && currentAudio) {
        ...
        const pageNum = currentPage + 1;
        const files = [];
        if (langMode === 'both' || langMode === 'zh') files.push(`/bear-kitchen/audio/page-${pageNum}-zh.mp3`);
        if (langMode === 'both' || langMode === 'en') files.push(`/bear-kitchen/audio/page-${pageNum}-en.mp3`);
...
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.register('/bear-kitchen/sw.js', { updateViaCache: 'none' });
}
