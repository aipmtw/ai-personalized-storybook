const CACHE_NAME = 'storybook-time-travel-v4';

// Generate audio file URLs for all 11 pages (cover + 10 story pages)
const AUDIO_URLS = [];
for (let i = 1; i <= 11; i++) {
  AUDIO_URLS.push(`/time-travel/audio/page-${i}-zh.mp3`);
  AUDIO_URLS.push(`/time-travel/audio/page-${i}-en.mp3`);
}
  ...
  if (event.request.mode === 'navigate' || event.request.url.endsWith('.html') || event.request.url.includes('/shared/')) {
    event.respondWith(
      fetch(event.request).then((response) => {
        ...
        return response;
      }).catch(() => caches.match(event.request) || caches.match('/time-travel/book.html'))
    );
    return;
  }
