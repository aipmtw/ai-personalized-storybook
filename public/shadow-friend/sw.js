const CACHE_NAME = 'storybook-shadow-friend-v3';

// Generate audio file URLs for all 10 pages
const AUDIO_URLS = [];
for (let i = 1; i <= 10; i++) {
  AUDIO_URLS.push(`/shadow-friend/audio/page-${i}-zh.mp3`);
  AUDIO_URLS.push(`/shadow-friend/audio/page-${i}-en.mp3`);
}
  ...
  if (event.request.mode === 'navigate' || event.request.url.endsWith('.html')) {
    event.respondWith(
      fetch(event.request).then((response) => {
        ...
        return response;
      }).catch(() => caches.match(event.request) || caches.match('/shadow-friend/book.html'))
    );
    return;
  }
