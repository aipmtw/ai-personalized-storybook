...
const pages = [
  ...
  },
  {
    ...
    emoji: '🏡🌾🌙',
    zh: '夏天的晚上，小光和媽媽一起到鄉下的阿嬤家。天空好暗好暗，星星好亮好亮。',
    en: 'On a summer night, Xiao Guang and Mom went to Grandma\'s countryside house. The sky was so dark, and the stars were so bright.'
  },
  {
    type: 'story',
    bg: 'page-bg-3',
    ...
    emoji: '👦💡🌿',
    zh: '小光在花園裡玩的時候，突然看到草叢裡有好多小小的光在飛來飛去。「那是什麼呀？」',
    en: 'While playing in the garden, Xiao Guang suddenly saw many tiny lights flying around in the bushes. "What are those?"'
  },
  {
    type: 'story',
    bg: 'page-bg-4',
    ...
    emoji: '👵✨😊',
    zh: '阿嬤笑著說：「那是螢火蟲呀！牠們的肚子會發光喔。」小光瞪大了眼睛，覺得好神奇。',
    en: 'Grandma smiled and said, "Those are fireflies! Their tummies can glow." Xiao Guang opened his eyes wide in wonder.'
  },
  {
    type: 'story',
    bg: 'page-bg-5',
    ...
    emoji: '👵👦🌾🌙',
    zh: '阿嬤牽著小光的手，一起走進黑黑的田野裡。四周好安靜，只聽到蟲鳴和風聲。',
    en: 'Grandma held Xiao Guang\'s hand and they walked together into the dark field. It was so quiet — only the sound of crickets and the wind.'
  },
  {
    type: 'story',
    bg: 'page-bg-6',
    ...
    emoji: '✨✨✨🌟✨✨',
    zh: '忽然間，好幾百隻螢火蟲飛了出來，把他們圍住了！就像滿天的星星降落在身邊。',
    en: 'Suddenly, hundreds of fireflies flew out and surrounded them! It was like stars falling all around them.'
  },
  {
    type: 'story',
    bg: 'page-bg-7',
    ...
    emoji: '👆✨🫢',
    zh: '一隻小小的螢火蟲輕輕停在小光的手指上。牠一閃一閃的，好像在跟小光打招呼。',
    en: 'A tiny firefly gently landed on Xiao Guang\'s finger. It flickered softly, as if saying hello.'
  },
  {
    type: 'story',
    bg: 'page-bg-8',
    ...
    emoji: '👵💛✨',
    zh: '阿嬤說：「螢火蟲發光是為了找朋友喔。每一道光，都是一個想交朋友的小心願。」',
    en: 'Grandma said, "Fireflies glow to find friends. Each little light is a tiny wish to make a friend."'
  },
  {
    type: 'story',
    bg: 'page-bg-9',
    ...
    emoji: '🌊✨🪷🌙',
    zh: '他們跟著螢火蟲走，來到了一個秘密的小池塘。水面倒映著螢火蟲的光，像一面星星做的鏡子。',
    en: 'They followed the fireflies and found a secret little pond. The water reflected the fireflies\' light, like a mirror made of stars.'
  },
  {
    type: 'end',
    bg: 'page-bg-10',
    ...
    emoji: '👵👦🌙💛',
    zh: '回家的路上，小光牽著阿嬤的手，輕輕地說：「螢火蟲，晚安。」阿嬤微笑著，把小光抱得更緊了。',
    en: 'Walking home hand in hand, Xiao Guang whispered, "Goodnight, fireflies." Grandma smiled and held him a little tighter.'
  }
];
...
function showDemoGate() {
  ...
  gate.id = 'demoGate';
  gate.style.cssText = 'position:fixed;inset:0;z-index:200;background:rgba(0,0,0,0.8);display:flex;align-items:center;justify-content:center;backdrop-filter:blur(6px)';
  gate.innerHTML = `<div style="background:#1e2e1e;border-radius:20px;padding:2.5rem;max-width:380px;text-align:center;margin:1rem;border:1px solid rgba(255,214,0,0.15)">
    <div style="font-size:2.5rem;margin-bottom:.8rem">🔒</div>
    <h2 style="font-size:1.3rem;font-weight:800;margin-bottom:.5rem;color:#f0f0e8">試閱結束</h2>
    <p style="font-size:1rem;color:#c8c8b8;line-height:1.7;margin-bottom:1.5rem">免費試閱前 3 頁已結束。<br>用 LINE 登入即可閱讀完整繪本！</p>
    ...
    <br><a href="#" onclick="document.getElementById('demoGate').style.display='none';goToPage(0);return false" style="font-size:.85rem;color:#8a8a7a;text-decoration:underline">返回第一頁</a>
  </div>`;
...
function updateUI() {
  ...
  // Update theme color per page — dark greens for nighttime
  const colors = ['#1a3a1a','#1b2a1b','#1e2e1e','#0f1f14','#0d1a14','#1a2a10','#1e2e1a','#0d1f1a','#0f1a1e','#15201e'];
  document.querySelector('meta[name="theme-color"]').content = colors[currentPage] || '#1b2a1b';
}
...
function toggleAudio() {
  ...
  if (langMode === 'both' || langMode === 'zh') {
    files.push(`/firefly-night/audio/page-${pageNum}-zh.mp3`);
  }
  if (langMode === 'both' || langMode === 'en') {
    files.push(`/firefly-night/audio/page-${pageNum}-en.mp3`);
  }
  ...
  for (let i = 0; i < totalDurations.length; i++) {
    ...
    if (cumulative + d >= target) {
      ...
      if (i === currentFileIndex && currentAudio) {
        ...
        const pageNum = currentPage + 1;
        const files = [];
        if (langMode === 'both' || langMode === 'zh') files.push(`/firefly-night/audio/page-${pageNum}-zh.mp3`);
        if (langMode === 'both' || langMode === 'en') files.push(`/firefly-night/audio/page-${pageNum}-en.mp3`);
...
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.register('/firefly-night/sw.js', { updateViaCache: 'none' });
}
