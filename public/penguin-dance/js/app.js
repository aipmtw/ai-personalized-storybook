...
const pages = [
  {
    ...
    en: '',
    coverTitle: '企鵝舞會',
    coverSubtitle: 'Penguin Dance Party',
  ...
  },
  {
    ...
    emoji: '🐧🏘️❄️',
    zh: '在遙遠的南極，有一個企鵝村莊。今天是一年一度的企鵝舞會！',
    en: 'Far away in the Antarctic, there was a penguin village. Today is the annual Penguin Dance Party!'
  },
  {
    type: 'story',
    bg: 'page-bg-3',
    ...
    emoji: '🐧😢💭',
    zh: '小企鵝澎澎很想參加，可是他不會跳舞。他躲在冰山後面，覺得好孤單。',
    en: 'Little penguin Peng Peng really wanted to join, but he couldn\'t dance. He hid behind an iceberg, feeling so lonely.'
  },
  {
    type: 'story',
    bg: 'page-bg-4',
    ...
    emoji: '🐧💫🤸',
    zh: '澎澎偷偷跑出來，學其他企鵝的動作。可是他一直滑倒，摔了好多跤！',
    en: 'Peng Peng sneaked out and tried to copy the other penguins\' moves. But he kept slipping and falling!'
  },
  {
    type: 'story',
    bg: 'page-bg-5',
    ...
    emoji: '🦭🐧🤝',
    zh: '一隻好心的海豹游過來說：「嘿！你不用學別人。用你的方式滑著跳吧！」',
    en: 'A kind seal swam over and said, "Hey! You don\'t have to copy others. Dance by sliding your own way!"'
  },
  {
    type: 'story',
    bg: 'page-bg-6',
    ...
    emoji: '🐧🛷✨',
    zh: '澎澎靈機一動，發明了獨一無二的「肚皮滑行舞」！他用肚子在冰上滑來滑去，好好玩！',
    en: 'Peng Peng had a great idea and invented his own special "Belly Slide Dance"! He slid around on the ice on his tummy — so much fun!'
  },
  {
    type: 'story',
    bg: 'page-bg-7',
    ...
    emoji: '🐧🐧🐧🎶',
    zh: '所有企鵝都看呆了！他們紛紛跟著澎澎一起滑，大家都好開心。',
    en: 'All the penguins were amazed! They all started sliding along with Peng Peng. Everyone was so happy!'
  },
  {
    type: 'story',
    bg: 'page-bg-8',
    ...
    emoji: '🌌🐧💃',
    zh: '他們一起在美麗的極光下跳舞。冰面閃閃發亮，就像一個超大的迪斯乖乖舞池！',
    en: 'They all danced together under the beautiful aurora. The ice sparkled and shimmered like a giant disco ball!'
  },
  {
    type: 'story',
    bg: 'page-bg-9',
    ...
    emoji: '🐧🌟💪',
    zh: '澎澎終於明白了：「跟別人不一樣，才是我最厲害的超能力！」',
    en: 'Peng Peng finally understood: "Being different is my greatest superpower!"'
  },
  {
    type: 'end',
    bg: 'page-bg-10',
    ...
    emoji: '🐧🎉❄️',
    zh: '這是企鵝村有史以來最棒的舞會。澎澎和朋友們跳到星星都在鼓掌！',
    en: 'It was the best dance party the penguin village had ever seen. Peng Peng and his friends danced until the stars applauded!'
  }
];
...
function updateUI() {
  ...
  const colors = ['#26c6da','#b3e5fc','#80cbc4','#4fc3f7','#4dd0e1','#26a69a','#7c4dff','#e1f5fe','#ffab40','#7c4dff'];
  document.querySelector('meta[name="theme-color"]').content = colors[currentPage] || '#26c6da';
}
...
function toggleAudio() {
  ...
  if (langMode === 'both' || langMode === 'zh') {
    files.push(`/penguin-dance/audio/page-${pageNum}-zh.mp3`);
  }
  if (langMode === 'both' || langMode === 'en') {
    files.push(`/penguin-dance/audio/page-${pageNum}-en.mp3`);
  }
  ...
  for (let i = 0; i < totalDurations.length; i++) {
    ...
    if (cumulative + d >= target) {
      ...
      if (i === currentFileIndex && currentAudio) {
        ...
        const pageNum = currentPage + 1;
        const files = [];
        if (langMode === 'both' || langMode === 'zh') files.push(`/penguin-dance/audio/page-${pageNum}-zh.mp3`);
        if (langMode === 'both' || langMode === 'en') files.push(`/penguin-dance/audio/page-${pageNum}-en.mp3`);
...
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.register('/penguin-dance/sw.js', { updateViaCache: 'none' });
}
